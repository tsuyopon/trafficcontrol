...
# traffic_monitorは「/api/4.0/cdns/CDN-in-a-Box/snapshot」のスナップショットが取得できるまでは起動しません。
# AUTO_SNAPQUEUE_ENABLED=true(デフォルト)の場合にはto-auto-snapqueueがtrafficopsで実行されるので、それが完了してTrafficOpsで取得できるようになるまで待ちます
...
# デバッグが有効化(TM_DEBUG_ENABLE=true)どうかで待ち時間を分離します。
...
# 対象のtemplateファイル(traffic_monitor.cfg.template)に環境変数の埋め込みを行い、traffic_monitor.cfgとして出力します。
envsubst < /opt/traffic_monitor/conf/traffic_monitor.cfg.template > /opt/traffic_monitor/conf/traffic_monitor.cfg

# 下記でtraffic_monitorを起動します。
...
# コンテナが終了しないように/dev/nullをtail -fします。
